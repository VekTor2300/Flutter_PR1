...
class DataBaseHelper{
  ...
  Future<void> onInitTable(Database db) async{
    try{
      db.insert(DataBaseRequest.tableUsers,User(login: 'admin1',password: md5.convert(utf8.encode('admin123')).toString(),id_role: RoleEnum.admin.id,).toMap());
      
      db.insert(DataBaseRequest.tableRole,Role(role: 'Администратор').toMap());
      db.insert(DataBaseRequest.tableRole,Role(role: 'Без роли').toMap());
      db.insert(DataBaseRequest.tableRole,Role(role: 'Поьзователь').toMap());
